...
describe('MenuBuilder', () => {
  ...
  beforeEach(() => {
    electron = {
      ...
      app: { getName: () => ('MediBloc Wallet'), getVersion: () => ('0.2.0') },
  ...
  it('Builds the electron about menu when os is mac', () => {
    ...
    expect(template[0].label).to.equal('MediBloc Wallet');
  ...
  it('Builds the electron about menu when os is not mac', () => {
    ...
    expect(template[0].label).to.not.equal('MediBloc Wallet');
    ...
    const expectedOptions = {
      buttons: ['OK'],
      icon: '//assets/images/MEDIBLOC.png',
  ...
  it('Should open link on click', () => {
    menu.onClickLink(electron, 'https://medibloc.org');
    expect(electron.shell.openExternal).to.have.been.calledWith('https://medibloc.org');
  });
});
